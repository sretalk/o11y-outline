...
- **指标**：系统的流量、成功率、延迟，所在宿主机的 CPU、内存利用率，所依赖的数据库的连接数、响应时间等。
...
[Honeycomb.io](https://www.honeycomb.io/) 是宽事件的倡导者和践行者，其产品设计也围绕宽事件展开。比如要统计某个接口的 P95 延迟，不是计算 Histogram 类型的指标，而是直接查询宽事件数据，计算 P95。
...
![可观测性建设路径](../path/path.png)

根据笔者的不精确观察，大部分企业都达成了第一阶段，第二阶段达成率不超过30%，第三阶段达成率不超过10%，第四阶段更少。

可观测性体系的建设，是一个长期迭代的持久战，不是说引入了一个工具就万事大吉了，而且细节繁多，必须要有专人负责，持续推进，有老板的持续支持，才更容易成功。后面抽一个章节更详细介绍这个话题。
